...
$t->check('Accessing an unloaded instance throws exception',$ex);
...
$t->check('Accessing a reloaded instance throws exception',$ex);

$t->check('Reloaded instance is not valid',!$map1->is_valid());
...
$t->check('check() returns no error',Automap_Tools::check($map1)===0);
...
function success_func($entry,$map)
{
$t=$GLOBALS['t'];
$stype=$entry['stype'];
$sname=$entry['symbol'];
